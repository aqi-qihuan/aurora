...
type UpdateUserVO struct {
	Nickname *string `json:"nickname,omitempty" max=30`
	Intro    *string `json:"intro,omitempty" max=500`
	WebSite  *string `json:"webSite,omitempty" max=255"`
	Avatar   *string `json:"avatar,omitempty" max=1024`
}
...
type CategoryVO struct {
	...
	Alias        string `json:"alias,omitempty" max=50`
...
type FriendLinkVO struct {
	...
	LinkAvatar  string `json:"linkAvatar" max=1024`
	LinkAddress string `json:"linkAddress" binding:"required,url,max=500"`
	LinkIntro   string `json:"linkIntro" max=500`
}
...
type PhotoAlbumVO struct {
	...
	AlbumCover string `json:"albumCover,omitempty" max=1024"`
	Info      string `json:"info,omitempty" max=500"`
...
type MenuVO struct {
	...
	Component  string  `json:"component,omitempty" max=255"`
	Icon       string  `json:"icon,omitempty" max=100"`
	Sort       int     `json:"sort"`
	Type       int8    `json:"type" binding:"required,oneof=0 1 2"` // 0目录 1菜单 2按钮
	Permission string  `json:"permission,omitempty" max=100"`
...
type WebsiteConfigVO struct {
	SiteName             string `json:"siteName" max=50"`
	SiteURL              string `json:"siteUrl" max=255"`
	AuthorName           string `json:"authorName" max=30"`
	AuthorAvatar         string `json:"authorAvatar" max=1024"`
	Logo                 string `json:"logo" max=1024"`
	Favicon              string `json:"favicon" max=1024"`
	SiteIntro            string `json:"siteIntro" max=500"`
	Notice               string `json:"notice" max=500"`
	FooterInfo           string `json:"footerInfo" max=500"`
	IcpNumber            string `json:"icpNumber" max=50"`
	BaiduPushURL         string `json:"baiduPushUrl" max=255"`
	GAID                 string `json:"gaId" max=100"`
	WechatQRCode         string `json:"wechatQrcode" max=1024"`
	AlipayQRCode         string `json:"alipayQrcode" max=1024"`
