...
2. Docker 镜像使用 `eclipse-temurin:25-jre-alpine`
...
**解决方案**: 创建 `ElasticsearchIndexInitializer` 类，在应用启动时自动创建索引
...
**教训**: ES 索引中的字段名是 `id`，不是 `articleId`，从 ES 读取数据时必须使用实际的字段名
...
**原因**: ES 8.x 的 `JsonData.toJson()` 返回的是 `JsonValue` 对象，必须调用 `toString()` 获取 JSON 字符串
...
**解决方案**: 在 `ElasticsearchIndexInitializer` 中添加数据同步方法
...
**解决方案**: 在 `buildQuery` 方法中明确指定 analyzer
...
2. 在 `ElasticsearchIndexInitializer` 上添加 `@Profile("!test")`
...
1. **字段映射必须匹配实际存储**: ES 中的字段名必须与索引时的字段名一致
2. **数据同步机制**: MaxWell 只同步增量变更，删除索引后需要手动全量同步历史数据
3. **分词器配置**: 索引时使用 `ik_max_word`，搜索时使用 `ik_smart`，查询时必须明确指定 analyzer
4. **高亮配置**: 全局设置 `preTags` 和 `postTags`，字段级别可以单独配置 `fragmentSize` 和 `numberOfFragments`
5. **测试注意事项**: ES 搜索测试需要真实环境，避免对"不存在的关键词"做强断言
