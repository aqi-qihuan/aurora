<template>
  <el-card class="main-card">
    <div class="title">{{ route.meta.title || route.name || '异常日志' }}</div>
    ...
    <el-dialog v-model="isCheck" append-to-body top="20px" width="80%" destroy-on-close>
      ...
      <el-form :model="exceptionLog" label-width="100px" size="small">
        ...
        <div>
          <pre><code class="language-java">{{ exceptionInfo }}</code></pre>
        </div>
      </el-form>
    </el-dialog>
...
const currentChange = (val) => {
  current.value = val
  pageStateStore.updateExceptionLogPageState(val)
  listLogs()
}

const searchLogs = () => {
  current.value = 1
  pageStateStore.updateExceptionLogPageState(current.value)
  listLogs()
}
...
const check = (log) => {
  ...
  isCheck.value = true
  nextTick(() => {
    // Prism.highlightAll() // 如果需要代码高亮，确保引入 Prism
  })
}
</script>
